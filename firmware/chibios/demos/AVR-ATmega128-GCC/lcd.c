...
void lcdCmd(uint8_t cmd) {
	...
	e_pulse();
	PORTC = (PORTC | PORTC_44780_DATA_MASK) &
					((cmd << 4) | (0x0F & ~PORTC_44780_RS_MASK));
	e_pulse();
}
...
void lcdPutc(char c) {
	...
	wait_not_busy();
	b = c | 0x0F;
	PORTC = (PORTC | PORTC_44780_DATA_MASK | PORTC_44780_RS_MASK) & (c | 0x0F);
	e_pulse();
	PORTC =
			(PORTC | PORTC_44780_DATA_MASK | PORTC_44780_RS_MASK) & ((c << 4) | 0x0F);
	e_pulse();
}
