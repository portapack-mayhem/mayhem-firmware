...
static void h1(eventid_t id) {
	(void)id;
	test_emit_token('A');
}
static void h2(eventid_t id) {
	(void)id;
	test_emit_token('B');
}
static void h3(eventid_t id) {
	(void)id;
	test_emit_token('C');
}
...
ROMCONST struct testcase testevt1 = {"Events, registration and dispatch",
																		 evt1_setup, NULL, evt1_execute};
...
static void evt2_execute(void) {
	...
	target_time = chTimeNow() + MS2ST(50);
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() - 1, thread2, "A");
	m = chEvtWaitAll(5);
...
ROMCONST struct testcase testevt2 = {"Events, wait and broadcast", evt2_setup,
																		 NULL, evt2_execute};
...
ROMCONST struct testcase testevt3 = {"Events, timeouts", evt3_setup, NULL,
																		 evt3_execute};
