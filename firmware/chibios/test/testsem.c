...
static void sem1_execute(void) {
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() + 5, thread1, "A");
	threads[1] =
			chThdCreateStatic(wa[1], WA_SIZE, chThdGetPriority() + 1, thread1, "B");
	threads[2] =
			chThdCreateStatic(wa[2], WA_SIZE, chThdGetPriority() + 3, thread1, "C");
	threads[3] =
			chThdCreateStatic(wa[3], WA_SIZE, chThdGetPriority() + 4, thread1, "D");
	threads[4] =
			chThdCreateStatic(wa[4], WA_SIZE, chThdGetPriority() + 2, thread1, "E");
	chSemSignal(&sem1);
	chSemSignal(&sem1);
	chSemSignal(&sem1);
	chSemSignal(&sem1);
...
ROMCONST struct testcase testsem1 = {"Semaphores, enqueuing", sem1_setup, NULL,
																		 sem1_execute};
...
static void sem2_execute(void) {
	 ...
	 */
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() - 1, thread2, 0);
...
ROMCONST struct testcase testsem2 = {"Semaphores, timeout", sem2_setup, NULL,
																		 sem2_execute};
...
static void sem3_execute(void) {
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() + 1, thread3, 0);
...
ROMCONST struct testcase testsem3 = {"Semaphores, atomic signal-wait",
																		 sem3_setup, NULL, sem3_execute};
#endif /* CH_USE_SEMSW */
...
static void sem4_execute(void) {
	...
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() - 1, thread4, &bsem);
...
ROMCONST struct testcase testsem4 = {"Binary Semaphores, functionality", NULL,
																		 NULL, sem4_execute};
