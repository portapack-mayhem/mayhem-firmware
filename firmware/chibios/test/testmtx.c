...
ROMCONST struct testcase testmtx1 = {"Mutexes, priority enqueuing test",
																		 mtx1_setup, NULL, mtx1_execute};
...
static void mtx2_execute(void) {
	...
	time = chTimeNow();
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() - 1, thread2H, 0);
	threads[1] =
			chThdCreateStatic(wa[1], WA_SIZE, chThdGetPriority() - 2, thread2M, 0);
	threads[2] =
			chThdCreateStatic(wa[2], WA_SIZE, chThdGetPriority() - 3, thread2L, 0);
	test_wait_threads();
	test_assert_sequence(1, "ABC");
	test_assert_time_window(2, time + MS2ST(100),
													time + MS2ST(100) + ALLOWED_DELAY);
}

ROMCONST struct testcase testmtx2 = {
		"Mutexes, priority inheritance, simple case", mtx2_setup, NULL,
		mtx2_execute};
...
static void mtx3_execute(void) {
	...
	time = chTimeNow();
	threads[0] =
			chThdCreateStatic(wa[0], WA_SIZE, chThdGetPriority() - 5, thread3LL, 0);
	threads[1] =
			chThdCreateStatic(wa[1], WA_SIZE, chThdGetPriority() - 4, thread3L, 0);
	threads[2] =
			chThdCreateStatic(wa[2], WA_SIZE, chThdGetPriority() - 3, thread3M, 0);
	threads[3] =
			chThdCreateStatic(wa[3], WA_SIZE, chThdGetPriority() - 2, thread3H, 0);
	threads[4] =
			chThdCreateStatic(wa[4], WA_SIZE, chThdGetPriority() - 1, thread3HH, 0);
	test_wait_threads();
	test_assert_sequence(1, "ABCDE");
	test_assert_time_window(2, time + MS2ST(110),
													time + MS2ST(110) + ALLOWED_DELAY);
}

ROMCONST struct testcase testmtx3 = {
		"Mutexes, priority inheritance, complex case", mtx3_setup, NULL,
		mtx3_execute};
...
ROMCONST struct testcase testmtx4 = {"Mutexes, priority return", mtx4_setup,
																		 NULL, mtx4_execute};
...
ROMCONST struct testcase testmtx5 = {"Mutexes, status", mtx5_setup, NULL,
																		 mtx5_execute};
...
ROMCONST struct testcase testmtx6 = {"CondVar, signal test", mtx6_setup, NULL,
																		 mtx6_execute};
...
ROMCONST struct testcase testmtx7 = {"CondVar, broadcast test", mtx7_setup,
																		 NULL, mtx7_execute};
...
ROMCONST struct testcase testmtx8 = {"CondVar, boost test", mtx8_setup, NULL,
																		 mtx8_execute};
