...
static void heap1_execute(void) {
	...
	test_assert(6,
							(chHeapStatus(&test_heap, &n) == 1) ||
									(chHeapStatus(&test_heap, &n) == 2),
							"heap fragmented");
	chHeapFree(p2);
	chHeapFree(p1);
...
ROMCONST struct testcase testheap1 = {"Heap, allocation and fragmentation test",
																			heap1_setup, NULL, heap1_execute};
