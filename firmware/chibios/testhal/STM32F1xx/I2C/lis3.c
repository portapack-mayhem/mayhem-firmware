...
int init_lis3(void) {
	...
	accel_tx_data[0] =
			ACCEL_CTRL_REG1 | AUTO_INCREMENT_BIT; /* register address */
	...
	/* sending */
	i2cAcquireBus(&I2CD1);
	status = i2cMasterTransmitTimeout(&I2CD1, lis3_addr, accel_tx_data, 4,
																		accel_rx_data, 0, tmo);
	i2cReleaseBus(&I2CD1);
...
void request_acceleration_data(void) {
	...
	i2cAcquireBus(&I2CD1);
	status = i2cMasterTransmitTimeout(&I2CD1, lis3_addr, accel_tx_data, 1,
																		accel_rx_data, 6, tmo);
	i2cReleaseBus(&I2CD1);
