 ...
 */
static const SerialUSBConfig serusbcfg = {&USBD1, USBD1_DATA_REQUEST_EP,
...
void cmd_sdiotest(BaseSequentialStream* chp, int argc, char* argv[]) {
	...
	if (!mmcConnect(&MMCD1)) {
		...
		chprintf(chp, "%u-%u-%u %u:%u:%u\r\n", timp.tm_year + 1900, timp.tm_mon + 1,
		...
		chprintf(chp, "------------------------------------------------------\r\n");
		chprintf(chp,
						 "Now you can remove memory card and check timestamp on PC.\r\n");
		chThdSleepMilliseconds(100);
	} else {
		chSysHalt();
	}
...
static const ShellCommand commands[] = {{"sdiotest", cmd_sdiotest},
																				{NULL, NULL}};
