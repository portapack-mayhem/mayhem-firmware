...
msg_t ThreadReference::suspend(void) {
	...
	chDbgAssert(thread_ref != NULL, "ThreadReference, #1", "already referenced");
...
msg_t ThreadReference::suspendS(void) {
	chDbgAssert(thread_ref == NULL, "ThreadReference, #2", "already referenced");
...
void ThreadReference::resume(msg_t msg) {
	chSysLock()
			...
			chDbgAssert(thread_ref != NULL, "ThreadReference, #3", "not referenced");
...
void ThreadReference::resumeI(msg_t msg) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #4", "not referenced");
...
void ThreadReference::requestTerminate(void) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #5", "not referenced");
...
msg_t ThreadReference::wait(void) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #6", "not referenced");
...
msg_t ThreadReference::sendMessage(msg_t msg) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #7", "not referenced");
...
msg_t ThreadReference::getMessage(void) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #8", "not referenced");

	return chMsgGet(thread_ref);
}

void ThreadReference::releaseMessage(msg_t msg) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #9", "not referenced");
...
void ThreadReference::signalEvents(eventmask_t mask) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #10", "not referenced");

	chEvtSignal(thread_ref, mask);
}

void ThreadReference::signalEventsI(eventmask_t mask) {
	chDbgAssert(thread_ref != NULL, "ThreadReference, #11", "not referenced");
...
void EvtSource::registerOne(chibios_rt::EvtListener* elp, eventid_t eid) {
	chEvtRegister(&ev_source, &elp->ev_listener, eid);
}

void EvtSource::registerMask(chibios_rt::EvtListener* elp, eventmask_t emask) {
...
size_t OutQueue::writeTimeout(const uint8_t* bp, size_t n, systime_t time) {
