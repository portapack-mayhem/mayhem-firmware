...
static const edma_channel_config_t adc_rfifo0_dma_config = {
		1,
		SPC5_ADC_FIFO0_DMA_PRIO,
		SPC5_ADC_FIFO0_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD1};
...
static const edma_channel_config_t adc_rfifo1_dma_config = {
		3,
		SPC5_ADC_FIFO1_DMA_PRIO,
		SPC5_ADC_FIFO1_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD2};
...
static const edma_channel_config_t adc_rfifo2_dma_config = {
		5,
		SPC5_ADC_FIFO2_DMA_PRIO,
		SPC5_ADC_FIFO2_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD3};
...
static const edma_channel_config_t adc_rfifo3_dma_config = {
		7,
		SPC5_ADC_FIFO3_DMA_PRIO,
		SPC5_ADC_FIFO3_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD4};
...
static const edma_channel_config_t adc_rfifo4_dma_config = {
		9,
		SPC5_ADC_FIFO4_DMA_PRIO,
		SPC5_ADC_FIFO4_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD5};
...
static const edma_channel_config_t adc_rfifo5_dma_config = {
		11,
		SPC5_ADC_FIFO5_DMA_PRIO,
		SPC5_ADC_FIFO5_DMA_IRQ_PRIO,
		adc_serve_rfifo_irq,
		adc_serve_dma_error_irq,
		&ADCD6};
...
static void adc_disable(void) {
	adc_write_register(EQADC_RW_BN_ADC0, ADC_REG_CR, SPC5_ADC_CR_CLK_PS);
	adc_write_register(EQADC_RW_BN_ADC1, ADC_REG_CR, SPC5_ADC_CR_CLK_PS);
}
...
void adc_lld_start_conversion(ADCDriver* adcp) {
	...
	edmaChannelSetup(
			...
			(uint32_t)adcp->grpp->num_channels * (uint32_t)adcp->depth, /* iter. */
			CPL2((uint32_t)adcp->grpp->num_channels * (uint32_t)adcp->depth *
	...
	SIU.ETISR.R = (SIU.ETISR.R & ~(3U << bitoff)) | (adcp->grpp->tsel << bitoff);
	bitoff = (uint32_t)adcp->fifo * 5;
	SIU.ISEL3.R =
			(SIU.ISEL3.R & ~(31U << bitoff)) | (adcp->grpp->etsel << bitoff);
	...
	cfifo_enable(
			adcp->fifo, adcp->grpp->cfcr,
			EQADC_IDCR_CFFE | EQADC_IDCR_CFFS | EQADC_IDCR_RFDE | EQADC_IDCR_RFDS);
}
