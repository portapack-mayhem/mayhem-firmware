...
static uint32_t usb_pm_alloc(USBDriver* usbp, size_t size) {
	...
	next = usbp->pmnext;
	usbp->pmnext += size;
	chDbgAssert(usbp->pmnext <= USB_PMA_SIZE, "usb_pm_alloc(), #1",
							"PMA overflow");
	return next;
}
		...
		if (epr & EPR_CTR_RX) {
			...
			if (epr & EPR_SETUP) {
				...
				if (epcp->out_state->rxqueued)
					usb_packet_read_to_queue(udp, epcp->out_state->mode.queue.rxqueue, n);
				else {
...
void usb_lld_reset(USBDriver* usbp) {
	...
	cntr = /*CNTR_ESOFM | */ CNTR_RESETM | CNTR_SUSPM | CNTR_WKUPM |
				 /*CNTR_ERRM | CNTR_PMAOVRM |*/ CNTR_CTRM;
...
void usb_lld_prepare_transmit(USBDriver* usbp, usbep_t ep) {
	...
	if (isp->txqueued)
		usb_packet_write_from_queue(USB_GET_DESCRIPTOR(ep), isp->mode.queue.txqueue,
																n);
	else
		usb_packet_write_from_buffer(USB_GET_DESCRIPTOR(ep), isp->mode.linear.txbuf,
																 n);
}
