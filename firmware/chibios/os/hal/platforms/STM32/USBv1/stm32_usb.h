...
#define EPR_TOGGLE_MASK \
	(EPR_STAT_TX_MASK | EPR_DTOG_TX | EPR_STAT_RX_MASK | EPR_DTOG_RX | EPR_SETUP)
...
#define EPR_SET_STAT_RX(ep, epr)                                       \
	STM32_USB->EPR[ep] =                                                 \
			((STM32_USB->EPR[ep] & ~(EPR_TOGGLE_MASK & ~EPR_STAT_RX_MASK)) ^ \
			 (epr)) |                                                        \
			EPR_CTR_MASK

#define EPR_SET_STAT_TX(ep, epr)                                       \
	STM32_USB->EPR[ep] =                                                 \
			((STM32_USB->EPR[ep] & ~(EPR_TOGGLE_MASK & ~EPR_STAT_TX_MASK)) ^ \
			 (epr)) |                                                        \
			EPR_CTR_MASK

#define EPR_CLEAR_CTR_RX(ep) \
	STM32_USB->EPR[ep] =       \
			(STM32_USB->EPR[ep] & ~EPR_CTR_RX & ~EPR_TOGGLE_MASK) | EPR_CTR_TX

#define EPR_CLEAR_CTR_TX(ep) \
	STM32_USB->EPR[ep] =       \
			(STM32_USB->EPR[ep] & ~EPR_CTR_TX & ~EPR_TOGGLE_MASK) | EPR_CTR_RX
...
#define USB_GET_DESCRIPTOR(ep)                                 \
	((stm32_usb_descriptor_t*)((uint32_t)STM32_USBRAM_BASE +     \
														 ...
														 (uint32_t)(ep) * sizeof(stm32_usb_descriptor_t)))
