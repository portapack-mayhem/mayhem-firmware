...
#define SPI1_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI1_RX_DMA_STREAM, STM32_SPI1_RX_DMA_CHN)

#define SPI1_TX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI1_TX_DMA_STREAM, STM32_SPI1_TX_DMA_CHN)

#define SPI2_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI2_RX_DMA_STREAM, STM32_SPI2_RX_DMA_CHN)

#define SPI2_TX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI2_TX_DMA_STREAM, STM32_SPI2_TX_DMA_CHN)

#define SPI3_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI3_RX_DMA_STREAM, STM32_SPI3_RX_DMA_CHN)

#define SPI3_TX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI3_TX_DMA_STREAM, STM32_SPI3_TX_DMA_CHN)

#define SPI4_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI4_RX_DMA_STREAM, STM32_SPI4_RX_DMA_CHN)

#define SPI4_TX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI4_TX_DMA_STREAM, STM32_SPI4_TX_DMA_CHN)

#define SPI5_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI5_RX_DMA_STREAM, STM32_SPI5_RX_DMA_CHN)

#define SPI5_TX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI5_TX_DMA_STREAM, STM32_SPI5_TX_DMA_CHN)

#define SPI6_RX_DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SPI_SPI6_RX_DMA_STREAM, STM32_SPI6_RX_DMA_CHN)
...
void spi_lld_init(void) {
	...
	SPID1.txdmamode = STM32_DMA_CR_CHSEL(SPI1_TX_DMA_CHANNEL) |
										STM32_DMA_CR_PL(STM32_SPI_SPI1_DMA_PRIORITY) |
										STM32_DMA_CR_DIR_M2P | STM32_DMA_CR_DMEIE |
										STM32_DMA_CR_TEIE;
#endif
...
void spi_lld_start(SPIDriver* spip) {
	...
	if (spip->state == SPI_STOP) {
		...
		if (&SPID1 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI1_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
			chDbgAssert(!b, "spi_lld_start(), #1", "stream already allocated");
			b = dmaStreamAllocate(spip->dmatx, STM32_SPI_SPI1_IRQ_PRIORITY,
		...
		if (&SPID2 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI2_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
			chDbgAssert(!b, "spi_lld_start(), #3", "stream already allocated");
			b = dmaStreamAllocate(spip->dmatx, STM32_SPI_SPI2_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_tx_interrupt,
														(void*)spip);
		...
		if (&SPID3 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI3_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
		...
		if (&SPID4 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI4_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
			chDbgAssert(!b, "spi_lld_start(), #7", "stream already allocated");
			b = dmaStreamAllocate(spip->dmatx, STM32_SPI_SPI4_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_tx_interrupt,
														(void*)spip);
		...
		if (&SPID5 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI5_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
			chDbgAssert(!b, "spi_lld_start(), #9", "stream already allocated");
			b = dmaStreamAllocate(spip->dmatx, STM32_SPI_SPI5_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_tx_interrupt,
														(void*)spip);
		...
		if (&SPID6 == spip) {
			bool_t b;
			b = dmaStreamAllocate(spip->dmarx, STM32_SPI_SPI6_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_rx_interrupt,
														(void*)spip);
			chDbgAssert(!b, "spi_lld_start(), #11", "stream already allocated");
			b = dmaStreamAllocate(spip->dmatx, STM32_SPI_SPI6_IRQ_PRIORITY,
														(stm32_dmaisr_t)spi_lld_serve_tx_interrupt,
														(void*)spip);
