...
#if STM32_CAN_USE_CAN2
	if (&CAND2 == canp) {
		chDbgAssert(CAND1.state != CAN_STOP, "can_lld_start(), #1",
								"CAN1 must be started");
...
void can_lld_stop(CANDriver* canp) {
	...
	if (canp->state == CAN_READY) {
		...
		if (&CAND1 == canp) {
#if STM32_CAN_USE_CAN2
			chDbgAssert(CAND2.state == CAN_STOP, "can_lld_stop(), #1",
									"CAN2 must be stopped");
#endif
...
void can_lld_transmit(CANDriver* canp,
	...
	if (ctfp->IDE)
		tir =
				((uint32_t)ctfp->EID << 3) | ((uint32_t)ctfp->RTR << 1) | CAN_TI0R_IDE;
	else
...
#if STM32_CAN_USE_CAN1
	chDbgAssert(CAND1.state == CAN_STOP, "canSTM32SetFilters(), #1",
							"invalid state");
#endif
#if STM32_CAN_USE_CAN2
	chDbgAssert(CAND2.state == CAN_STOP, "canSTM32SetFilters(), #2",
							"invalid state");
#endif
