...
static const USBEndpointConfig ep0config = {
		USB_EP_MODE_TYPE_CTRL, _usb_ep0setup,	 _usb_ep0in,
		_usb_ep0out,					 0x40,					 0x40,
		&ep0_state.in,				 &ep0_state.out, 1,
		ep0setup_buffer};

#if STM32_USB_USE_OTG1
static const stm32_otg_params_t fsparams = {STM32_USB_OTG1_RX_FIFO_SIZE / 4,
																						STM32_OTG1_FIFO_MEM_SIZE,
																						STM32_OTG1_ENDOPOINTS_NUMBER};
#endif

#if STM32_USB_USE_OTG2
static const stm32_otg_params_t hsparams = {STM32_USB_OTG2_RX_FIFO_SIZE / 4,
...
static void otg_rxfifo_handler(USBDriver* usbp) {
	...
	switch (sts & GRXSTSP_PKTSTS_MASK) {
		...
		case GRXSTSP_SETUP_DATA:
			...
			otg_fifo_read_to_buffer(usbp->otg->FIFO[0], usbp->epc[ep]->setup_buf, cnt,
															8);
			break;
		case GRXSTSP_OUT_DATA:
			...
			if (usbp->epc[ep]->out_state->rxqueued) {
				...
				otg_fifo_read_to_buffer(
						usbp->otg->FIFO[0], usbp->epc[ep]->out_state->mode.linear.rxbuf,
						cnt,
						usbp->epc[ep]->out_state->rxsize - usbp->epc[ep]->out_state->rxcnt);
	...
	while (TRUE) {
		...
		if (usbp->epc[ep]->in_state->txqueued) {
			...
			otg_fifo_write_from_queue(usbp->otg->FIFO[ep],
																usbp->epc[ep]->in_state->mode.queue.txqueue, n);
		} else {
...
static void otg_epin_handler(USBDriver* usbp, usbep_t ep) {
	...
	if ((epint & DIEPINT_TXFE) && (otgp->DIEPEMPMSK & DIEPEMPMSK_INEPTXFEM(ep))) {
...
void usb_lld_init(void) {
...
#if CH_DBG_FILL_THREADS
	{
		...
		_thread_memfill((uint8_t*)wsp + sizeof(Thread),
										(uint8_t*)wsp + sizeof(USBD1.wa_pump), CH_STACK_FILL_VALUE);
	}
#endif
#endif
...
#if CH_DBG_FILL_THREADS
	{
		...
		_thread_memfill((uint8_t*)wsp, (uint8_t*)wsp + sizeof(Thread),
										CH_THREAD_FILL_VALUE);
		_thread_memfill((uint8_t*)wsp + sizeof(Thread),
										(uint8_t*)wsp + sizeof(USBD2.wa_pump), CH_STACK_FILL_VALUE);
	}
#endif
#endif
}
...
void usb_lld_start(USBDriver* usbp) {
	...
	if (usbp->state == USB_STOP) {
		...
		if (usbp->thd_ptr == NULL)
			usbp->thd_ptr = usbp->thd_wait =
					chThdCreateI(usbp->wa_pump, sizeof usbp->wa_pump,
											 STM32_USB_OTG_THREAD_PRIO, usb_lld_pump, usbp);
		...
		otgp->GCCFG =
				GCCFG_NOVBUSSENS | GCCFG_VBUSASEN | GCCFG_VBUSBSEN | GCCFG_PWRDWN;
#else
		...
		if (usbp->config->sof_cb == NULL)
			...
			otgp->GINTMSK = GINTMSK_ENUMDNEM |
											GINTMSK_USBRSTM /*| GINTMSK_USBSUSPM |
...
void usb_lld_reset(USBDriver* usbp) {
	...
	otgp->DIEPTXF0 =
			DIEPTXF_INEPTXFD(ep0config.in_maxsize / 4) |
			DIEPTXF_INEPTXSA(otg_ram_alloc(usbp, ep0config.in_maxsize / 4));
}
...
void usb_lld_init_endpoint(USBDriver* usbp, usbep_t ep) {
	...
	if (usbp->epc[ep]->in_cb != NULL) {
		...
		otgp->DIEPTXF[ep - 1] =
				DIEPTXF_INEPTXFD(fsize) | DIEPTXF_INEPTXSA(otg_ram_alloc(usbp, fsize));
		otg_txfifo_flush(usbp, ep);

		otgp->ie[ep].DIEPCTL =
				ctl | DIEPCTL_TXFNUM(ep) | DIEPCTL_MPSIZ(usbp->epc[ep]->in_maxsize);
...
void usb_lld_prepare_transmit(USBDriver* usbp, usbep_t ep) {
	...
	if (isp->txsize == 0) {
		...
		usbp->otg->ie[ep].DIEPTSIZ =
				DIEPTSIZ_PKTCNT(pcnt) | DIEPTSIZ_XFRSIZ(isp->txsize);
	}
}
