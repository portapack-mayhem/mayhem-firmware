...
#define DMA_CHANNEL \
	STM32_DMA_GETCHANNEL(STM32_SDC_SDIO_DMA_STREAM, STM32_SDC_SDIO_DMA_CHN)
...
static bool_t sdc_lld_prepare_read(SDCDriver* sdcp,
	...
	if (n > 1) {
		...
		if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_READ_SINGLE_BLOCK, startblk,
																	 resp) ||
				MMCSD_R1_ERROR(resp[0]))
			return CH_FAILED;
	}
...
static bool_t sdc_lld_prepare_write(SDCDriver* sdcp,
	...
	if (n > 1) {
		...
		if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_WRITE_BLOCK, startblk,
																	 resp) ||
				MMCSD_R1_ERROR(resp[0]))
			return CH_FAILED;
	}
...
#if (defined(STM32F4XX) || defined(STM32F2XX))
	sdcp->dmamode |= STM32_DMA_CR_PFCTRL | STM32_DMA_CR_PBURST_INCR4 |
...
bool_t sdc_lld_send_cmd_short(SDCDriver* sdcp,
	...
	while (((sta = SDIO->STA) &
					(SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT | SDIO_STA_CCRCFAIL)) == 0)
		;
...
bool_t sdc_lld_read_aligned(SDCDriver* sdcp,
	...
	SDIO->MASK = SDIO_MASK_DCRCFAILIE | SDIO_MASK_DTIMEOUTIE |
							 SDIO_MASK_STBITERRIE | SDIO_MASK_RXOVERRIE | SDIO_MASK_DATAENDIE;
...
bool_t sdc_lld_write_aligned(SDCDriver* sdcp,
	...
	SDIO->ICR = STM32_SDIO_ICR_ALL_FLAGS;
	SDIO->MASK = SDIO_MASK_DCRCFAILIE | SDIO_MASK_DTIMEOUTIE |
							 SDIO_MASK_STBITERRIE | SDIO_MASK_TXUNDERRIE |
							 SDIO_MASK_DATAENDIE;
	SDIO->DLEN = n * MMCSD_BLOCK_SIZE;
