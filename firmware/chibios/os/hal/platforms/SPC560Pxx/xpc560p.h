 ...
 * DATE	   : 12.06.2012
 * AUTHOR       : B16991
 * HISTORY     : Changes: typo fixed in PSR1 register:SCP -> CSP, MCR:
 *PRESCALE->BITRATE (b16991) HISTORY     : Changes: typo fixed in ME register
 *MTC bit, SSCM fix, CMU changes(b16991) HISTORY     : Changes to CTU Module: CR
 *register (LC->FC), CLR changed to 24 bits  (b16991) HISTORY      : Modified to
 *add reserved space in CTU (b16991) HISTORY      :  Modified to support ADC on
 *Pictus cut 2 - do not distribute! (ttz778) HISTORY      :  Modified to support
 *CRC on Pictus cut 2 - do not distribute! (r60321) HISTORY      :  Modified to
 *support DSPI0 CS7&8 and new FlexPWM naming on Pictus cut 2 (r60321) HISTORY :
 *Modified to update MIDR1&2 registers and LINCR1-SFTM and LINESR-BDEF bit on
 *Pictus (r60321) HISTORY      :  Modified to update RGM, CFLASH & DFLASH
 *registers and FlexCAN & CTU Registers on Pictus (r60321) HISTORY      :
 *Modified to update DSPI Registers (FIFO deep) on Pictus  (b16991)
 *
 *****************************************************************
 * Copyright:
...
/*for "channel link" format TCD (when
...
typedef union uMBSSUTR {
	...
	struct {
		vuint16_t : 2;
		vuint16_t LAST_MB_SEG1 : 6; /* last message buffer control register for
