...
struct EDMA_TCD_STD_tag { /* for "standard" format TCDs (when
														 EDMA.TCD[x].CITER.E_LINK==BITER.E_LINK=0 &&
														 EDMA.EMLM=0 ) */
	/* 00 */
...
struct XBAR_tag {
	union {
		...
		struct {
			...
			vuint32_t : 1;
			vuint32_t MSTR1 : 3; /* Master 1 Priority - e200z4 Core load/store & Nexus
															port  */
			vuint32_t : 1;
			vuint32_t MSTR0 : 3; /* Master 0 Priority - e200z4 core Instruction */
		} B;
	} MPR0; /* Master Priority Register for Slave port 0 (Flash) @baseaddress +
						 0x00 */
	...
	} SGPCR0; /* Slave General Purpose Control Register 0 (Flash) @baseaddress +
							 0x10 */
	...
	} MPR1; /* Master Priority Register for Slave port 1 (EBI) @baseaddress +
						 0x100 */
	...
	} SGPCR1; /* Slave General Purpose Control Register 1 (EBI) @baseaddress +
							 0x110 */
	...
	} MPR2; /* Master Priority Register for Slave port 2 (RAM) @baseaddress +
						 0x200 */
	...
	} SGPCR2; /* Slave General Purpose Control Register 2 (RAM)@baseaddress +
							 0x210 */

	int32_t XBAR_reserved_0214[59]; /* 0x0214 - 0x02FF */

	/* Slave General Purpose Control Register 3 @baseaddress + 0x310 - not
	 * implemented */
...
struct FLASH_tag {
	...
	} BIUAPR; /*Bus Interface Unit Access Protection Register @baseaddress +
							 0x20*/
...
struct EBI_tag {
	...
	struct CS_tag
			CS[4]; /* EBI CS Registers (BR / OR) @baseaddress + 0x10 - 0x2C */

	uint32_t EBI_reserved0030[4]; /* 0x0030 - 0x003C */

	struct CAL_CS_tag CAL_CS[4]; /* EBI CAL_CS Registers (CAL_BR / CAL_OR)
																	@baseaddress + 0x40 - 0x5C */
};
...
struct SIU_tag {
	union {
		...
		struct {
			...
			vuint32_t FLASH_SIZE_1 : 4; /* Define major Flash memory size (see Table
																		 15-4 for details) */
			vuint32_t
					FLASH_SIZE_2 : 4; /* Define Flash memory size, small granularity  */
	...
	union {
		...
		struct {
			...
			vuint32_t : 7;		 /*  */
			vuint32_t
					SWT : 1; /* Software Watch Dog Timer Interrupt Flag, from platform */
			vuint32_t : 7;			 /*  */
	...
	} IFEER; /* External IRQ Falling-Edge Event Enable Regi (IFEER) @baseaddress +
							0x2C */
	...
	union {
		...
		struct {
			...
			vuint32_t CNBSTP : 1;	 /* FlexCAN B stop request. When asserted, a stop
																request is sent to the FlexCAN B */
			vuint32_t CNASTP : 1;	 /* FlexCAN A stop request. When asserted, a stop
																request is sent to the FlexCAN A */
			vuint32_t SPIDSTP : 1; /* DSPI D stop request. When asserted, a stop
			...
			vuint32_t : 6;				 /* Reserved */
			vuint32_t SCICSTP : 1; /* eSCI C stop request. When asserted, a stop
																request is sent to the eSCI C module. */
	...
	union {
		...
		struct {
			...
			vuint32_t CNBACK : 1;	 /* FlexCAN B stop acknowledge. When asserted,
																indicates that a stop acknowledge */
			vuint32_t CNAACK : 1;	 /* FlexCAN A stop acknowledge. When asserted,
																indicates that a stop acknowledge */
			vuint32_t SPIDACK : 1; /* DSPI D stop acknowledge. When asserted,
...
struct FMPLL_tag {
	...
	} ESYNCR1; /* Enhanced Synthesizer Control Register 1 (ESYNCR1) @baseaddress +
								0x0008 */
	...
	} ESYNCR2; /* Enhanced Synthesizer Control Register 2 (ESYNCR2) @baseaddress +
								0x000C */
	...
	} SYNFMMR; /* Synthesizer FM Modulation Register (SYNFMMR) @baseaddress +
								0x0018 */
};
...
struct ETPU_tag {
	...
	union {
		vuint32_t R;
		struct {
			vuint32_t ETPUMISCCMP : 32; /* Expected multiple input signature
																		 calculator compare register value.  */
		} B;
	...
	} TBCR_A; /* eTPU Time Base Configuration Register (ETPU_TBCR)@baseaddress +
							 0x20 */
	...
	} TB1R_A; /* eTPU Time Base 1 (TCR1) Visibility Register
							 (ETPU_TB1R)@baseaddress + 0x24 */
	...
	} TB2R_A; /* eTPU Time Base 2 (TCR2) Visibility Register
							 (ETPU_TB2R)@baseaddress + 0x28 */
	...
	} CISR_A; /* eTPU Channel Interrupt Status Register (ETPU_CISR)@baseaddress +
							 0x200 */
	...
	} CDTRSR_A; /* eTPU Channel Data Transfer Request Status Register
								 (ETPU_CDTRSR) @baseaddress + 0x210 */
	...
	} CIOSR_A; /* eTPU Channel Interrupt Overflow Status Register
	...
	} CDTROSR_A; /* eTPU Channel Data Transfer Request Overflow Status
	...
	} CIER_A; /* eTPU Channel Interrupt Enable Register (ETPU_CIER)@baseaddress +
							 0x240 */
	...
	} CDTRER_A; /* eTPU Channel Data Transfer Request Enable Register
	...
	} CPSSR_A; /* ETPUCPSSR - eTPU Channel Pending Service Status Register
								@baseaddress + 0x280  */
	...
	} CSSR_A; /* ETPUCSSR - eTPU Channel Service Status Register @baseaddress +
							 0x290  */
	...
	struct {
		...
		} CR; /* eTPU Channel n Configuration Register (ETPU_CnCR)@baseaddress +
						 0x400 */
		...
		} SCR; /* eTPU Channel n Status Control Register (ETPU_CnSCR)@baseaddress +
							0x404 */
		...
		} HSRR; /* eTPU channel host service request register
...
struct DSPI_tag {
	...
	} CTAR[8]; /* Clock and Transfer Attributes Registers @baseaddress + 0x0C -
								0x28 */
	...
	} RSER; /* DMA/Interrupt Request Select and Enable Register @baseaddress +
						 0x30 */
...
typedef union uMBSSUTR {
	...
	struct {
		vuint16_t : 1;
		vuint16_t LAST_MB_SEG1 : 7; /* last message buffer control register for
																	 message buffer segment 1 */
		vuint16_t : 1;
...
typedef union uPIFR0 {
	...
	struct {
		...
		vuint16_t TBVBIF : 1; /* Transmission across boundary on channel B Interrupt
														 Flag */
		vuint16_t TBVAIF : 1; /* Transmission across boundary on channel A Interrupt
														 Flag */
...
typedef union uPIFR1 {
	...
	struct {
		...
		vuint16_t
				PECFIF : 1; /* protocol engine communication failure interrupt flag */
...
typedef union uPIER0 {
	...
	struct {
		...
		vuint16_t
				INTLIE : 1; /* internal protocol error interrupt interrupt enable  */
		...
		vuint16_t
				MTXIE : 1; /* media access test symbol received interrupt enable */
		vuint16_t
				LTXBIE : 1; /* pdLatestTx violation on channel B interrupt enable */
		vuint16_t
				LTXAIE : 1; /* pdLatestTx violation on channel A interrupt enable */
		vuint16_t TBVBIE : 1; /* Transmission across boundary on channel B Interrupt
														 enable */
		vuint16_t TBVAIE : 1; /* Transmission across boundary on channel A Interrupt
														 enable */
...
typedef union uPIER1 {
	...
	struct {
		...
		vuint16_t
				PECFIE : 1; /* protocol engine communication failure interrupt enable */
...
typedef struct FR_tag {
	...
	volatile SYSBADHR_t SYSBADHR;
			/*system memory base address high register */ /*4        */
	volatile SYSBADLR_t SYSBADLR;
			/*system memory base address low register */							 /*6         */
	...
	volatile MBSSUTR_t MBSSUTR;
			/*message buffer segment size and utilization register */					 /*E  */
	...
	volatile MBIVEC_t MBIVEC;
			/*message buffer interrupt vector register */ /*22 */
	volatile CASERCR_t CASERCR;
			/*channel A status error counter register */ /*24 */
	volatile CBSERCR_t CBSERCR;
			/*channel B status error counter register */									/*26 */
	...
	volatile SFTCCSR_t SFTCCSR;
			/*sync frame table configuration, control, status register */ /*44 */
	volatile SFIDRFR_t SFIDRFR;
			/*sync frame ID rejection filter register */ /*46 */
	volatile SFIDAFVR_t SFIDAFVR;
			/*sync frame ID acceptance filter value regiater */ /*48 */
	volatile SFIDAFMR_t SFIDAFMR;
			/*sync frame ID acceptance filter mask register */ /*4A */
	volatile NMVR_t NMVR[6];
			/*network management vector registers (12 bytes) */								 /*4C */
	...
	volatile RFMIDAFVR_t RFMIDAFVR;
			/*receive fifo message ID acceptance filter value register */ /*90 */
	volatile RFMIAFMR_t RFMIAFMR;
			/*receive fifo message ID acceptance filter mask register */ /*92 */
	volatile RFFIDRFVR_t RFFIDRFVR;
			/*receive fifo frame ID rejection filter value register */ /*94 */
	volatile RFFIDRFMR_t RFFIDRFMR;
			/*receive fifo frame ID rejection filter mask register */ /*96 */
	volatile RFRFCFR_t RFRFCFR;
			/*receive fifo range filter configuration register */ /*98 */
	volatile RFRFCTR_t RFRFCTR;
			/*receive fifo range filter control register */ /*9A */
	volatile LDTXSLAR_t LDTXSLAR;
			/*last dynamic transmit slot channel A register */ /*9C */
	volatile LDTXSLBR_t LDTXSLBR;
			/*last dynamic transmit slot channel B register */					/*9E */
	...
	volatile MSG_BUFF_CCS_t MBCCS[128];
			/* message buffer configuration, control & status registers 0-31 */ /*100
																																					 */
} FR_tag_t;
