...
#else
#define AT91_ADC1_PRESCALE \
	((((MCK / 2) + (AT91_ADC1_CLOCK - 1)) / AT91_ADC1_CLOCK) - 1)
#endif
...
#define AT91_ADC1_MAINMODE                                                \
	(((AT91_ADC1_SHTM & 0x0F) << 24) | ((AT91_ADC1_STARTUP & 0x7F) << 16) | \
	 ((AT91_ADC1_PRESCALE & 0xFF) << 8) | AT91C_ADC_LOWRES_8_BIT)
#else
#define AT91_ADC1_MAINMODE                                                \
	(((AT91_ADC1_SHTM & 0x0F) << 24) | ((AT91_ADC1_STARTUP & 0x7F) << 16) | \
	 ((AT91_ADC1_PRESCALE & 0xFF) << 8) | AT91C_ADC_LOWRES_10_BIT)
#endif
...
#define adc_sleep() \
	ADCReg1->ADC_MR = \
			(AT91_ADC1_MAINMODE | AT91C_ADC_SLEEP_MODE | AT91C_ADC_TRGEN_DIS)
#define adc_wake()  \
	ADCReg1->ADC_MR = \
			(AT91_ADC1_MAINMODE | AT91C_ADC_SLEEP_NORMAL_MODE | AT91C_ADC_TRGEN_DIS)
#define adc_disable()                                        \
	{                                                          \
...
#define adc_clrint()             \
	{                              \
		uint32_t isr, dummy;         \
                                 \
		isr = ADCReg1->ADC_SR;       \
		if ((isr & AT91C_ADC_DRDY))  \
			dummy = ADCReg1->ADC_LCDR; \
		if ((isr & AT91C_ADC_EOC0))  \
			dummy = ADCReg1->ADC_CDR0; \
		if ((isr & AT91C_ADC_EOC1))  \
			dummy = ADCReg1->ADC_CDR1; \
		if ((isr & AT91C_ADC_EOC2))  \
			dummy = ADCReg1->ADC_CDR2; \
		if ((isr & AT91C_ADC_EOC3))  \
			dummy = ADCReg1->ADC_CDR3; \
		if ((isr & AT91C_ADC_EOC4))  \
			dummy = ADCReg1->ADC_CDR4; \
		if ((isr & AT91C_ADC_EOC5))  \
			dummy = ADCReg1->ADC_CDR5; \
		if ((isr & AT91C_ADC_EOC6))  \
			dummy = ADCReg1->ADC_CDR6; \
		if ((isr & AT91C_ADC_EOC7))  \
			dummy = ADCReg1->ADC_CDR7; \
		(void)dummy;                 \
	}
#define adc_stop() \
	{                \
...
/**
 * We must keep stack usage to a minimum - the default AT91SAM7 isr stack size
 * is very small. We sacrifice some speed and code size in order to achieve this
 * by accessing the structure and registers directly rather than through the
 * passed in pointers. This works because the AT91SAM7 supports only a single
 * ADC device (although with 8 channels).
 */
static void handleint(void) {
	...
	if (ADCD1.grpp) {
		/* ADC overflow condition, this could happen only if the DMA is unable to
		...
		if ((isr & AT91C_ADC_GOVRE)) {
			...
			if (ADCD1.grpp->circular) {
				...
				if (ADCD1.depth <= 1) {
					...
					ADCReg1->ADC_RNPR = (uint32_t)(
							ADCD1.samples + (ADCD1.depth / 2 * ADCD1.grpp->num_channels));
					ADCReg1->ADC_RNCR =
							(ADCD1.depth - ADCD1.depth / 2) * ADCD1.grpp->num_channels;
				}
...
void adc_lld_start(ADCDriver* adcp) {
	...
	if (adcp->state == ADC_STOP) {
		...
		/* We could stay in sleep mode provided total conversion rate < 44kHz but we
		 * can't guarantee that here */
		adc_wake();

		/* TODO: We really should perform a conversion here just to ensure that we
		 * are out of sleep mode */
	}
}
...
void adc_lld_start_conversion(ADCDriver* adcp) {
	...
	switch (ADCD1.grpp->trigger & ~ADC_TRIGGER_SOFTWARE) {
		case ADC_TRIGGER_TIMER:
				...
				if (i < (0x10000 << 0)) {
					ADCTimer1->TC_CMR =
							(AT91C_TC_ASWTRG_CLEAR | AT91C_TC_ACPC_CLEAR | AT91C_TC_ACPA_SET |
							 AT91C_TC_LDRA_RISING | AT91C_TC_WAVE | AT91C_TC_WAVESEL_UP_AUTO |
			...
			ADCReg1->ADC_MR = AT91_ADC1_MAINMODE | AT91C_ADC_SLEEP_NORMAL_MODE |
												AT91C_ADC_TRGEN_EN | AT91_ADC1_TIMERMODE;
			break;

		case ADC_TRIGGER_EXTERNAL:
/* Make sure the ADTRG pin is set as an input - assume pull-ups etc have already
 * been set */
...
#endif
			ADCReg1->ADC_MR = AT91_ADC1_MAINMODE | AT91C_ADC_SLEEP_NORMAL_MODE |
												AT91C_ADC_TRGEN_EN | AT91C_ADC_TRGSEL_EXT;
			break;
