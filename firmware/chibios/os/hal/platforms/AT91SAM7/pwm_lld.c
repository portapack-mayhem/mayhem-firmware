...
void pwm_lld_start(PWMDriver* pwmp) {
	...
	/* Steps:
		...
		3. Selection of the clock for each channel (CPRE field in the PWM_CMRx
		register)
		4. Configuration of the waveform alignment for each channel (CALG field in
		the PWM_CMRx register)
		5. Configuration of the output waveform polarity for each channel (CPOL in
		the PWM_CMRx register)
		6. Configuration of the period for each channel (CPRD in the PWM_CPRDx
		register). Writing in PWM_CPRDx Register is possible while the channel is
		disabled. After validation of the channel, the user must use PWM_CUPDx
		Register to update PWM_CPRDx
	...
	*/
	...
	if (mode & PWM_OUTPUT_PIN1) {
		pwmp->pins->pin[0].perab[0] =
				pwmp->pins->pin[0].portpin; /* Select A or B peripheral */
		pwmp->pins->pin[0].pio->PIO_PDR =
				pwmp->pins->pin[0].portpin; /* Turn PIO into PWM output */
		pwmp->pins->pin[0].pio->PIO_MDDR =
				pwmp->pins->pin[0].portpin; /* Turn off PIO multi-drive */
		if (mode & PWM_DISABLEPULLUP_PIN1)
			pwmp->pins->pin[0].pio->PIO_PPUDR =
					pwmp->pins->pin[0].portpin; /* Turn off PIO pullup */
		else
			pwmp->pins->pin[0].pio->PIO_PPUER =
					pwmp->pins->pin[0].portpin; /* Turn on PIO pullup */
	}
	...
	if ((mode & PWM_MCK_MASK) == PWM_MCK_DIV_CLKA) {
		if (!pwmp->config->frequency) {
			...
			if (mode & PWM_OUTPUT_CENTER)
				div >>= 1;
			for (pre = 0; div > 255 && pre < 10; pre++)
				div >>= 1;
			if (div > 255)
				div = 255;
			PWMC_M->PWMC_MR =
					(PWMC_M->PWMC_MR & 0xFFFF0000) | (pre << 8) | (div << 0);
		}
	} else if ((mode & PWM_MCK_MASK) == PWM_MCK_DIV_CLKB) {
		if (!pwmp->config->frequency) {
			/* As slow as we go */
			PWMC_M->PWMC_MR =
					(PWMC_M->PWMC_MR & 0x0000FFFF) | (10 << 24) | (255 << 16);
		} else if (pwmp->config->frequency > MCK) {
			/* Just use MCLK */
			mode &= ~PWM_MCK_MASK;
		} else {
			...
			if (div > 255)
				div = 255;
			PWMC_M->PWMC_MR =
					(PWMC_M->PWMC_MR & 0x0000FFFF) | (pre << 24) | (div << 16);
		}
	}
...
void pwm_lld_stop(PWMDriver* pwmp) {
	...
	/* Turn the pin back to a PIO pin - we have forgotten pull-up and multi-drive
	 * state for the pin though */
	...
	if ((pwmp->config->channels[0].mode & PWM_OUTPUT_PIN3) &&
			pwmp->pins->pin[2].portpin)
...
void pwm_lld_enable_channel(PWMDriver* pwmp,
														...
														pwmcnt_t width) {
	/*
		6. Configuration of the duty cycle for each channel (CDTY in the PWM_CDTYx
		register). Writing in PWM_CDTYx Register is possible while the channel is
		disabled. After validation of the channel, the user must use PWM_CUPDx
		Register to update PWM_CDTYx.
