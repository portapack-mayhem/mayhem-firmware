...
msg_t i2c_lld_master_receive_timeout(I2CDriver* i2cp,
	...
	/* In single data byte master read or write, the START and STOP must both be
	 * set. */
	if (rxbytes == 1)
 ...
 * @notapi
 */
msg_t i2c_lld_transceive_timeout(I2CDriver* i2cp,
																 i2caddr_t addr,
																 const uint8_t* txbuf,
																 size_t txbytes,
																 uint8_t* rxbuf,
...
msg_t i2c_lld_master_transmit_timeout(I2CDriver* i2cp,
	...
	/* SAM7 specific check */
	chDbgCheck(
			((rxbytes == 0) || ((txbytes > 0) && (txbytes < 4) && (rxbuf != NULL))),
