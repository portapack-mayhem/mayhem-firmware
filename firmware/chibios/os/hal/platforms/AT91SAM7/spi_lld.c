...
void spi_lld_start(SPIDriver* spip) {
	if (spip->state == SPI_STOP) {
		...
		if (&SPID1 == spip) {
			...
			if (spip->config->ssport == AT91C_BASE_PIOA &&
					spip->config->sspad == SPI0_CS0) {
		...
		if (&SPID2 == spip) {
			...
			// Setup the chip select pin
			// A bug in SPI1 requires that the CS be automatically controlled if it
			// matches the CS0 pin.
			//	If it is not automatically controlled SPI1 will turn off when data is
			//written. 	If you are not using CS0 make absolutely sure you don't use it
			//as an output pin and 	clear it otherwise you will get the bug in anyway.
			if (spip->config->ssport == AT91C_BASE_PIOA &&
					spip->config->sspad == SPI1_CS0) {
