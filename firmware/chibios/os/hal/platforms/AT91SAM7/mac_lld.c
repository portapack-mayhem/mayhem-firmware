...
#define EMAC_PIN_MASK                                                       \
	(AT91C_PB0_ETXCK_EREFCK | AT91C_PB1_ETXEN | AT91C_PB2_ETX0 |              \
	 AT91C_PB3_ETX1 | AT91C_PB4_ECRS | AT91C_PB5_ERX0 | AT91C_PB6_ERX1 |      \
	 AT91C_PB7_ERXER | AT91C_PB8_EMDC | AT91C_PB9_EMDIO | AT91C_PB10_ETX2 |   \
	 AT91C_PB11_ETX3 | AT91C_PB12_ETXER | AT91C_PB13_ERX2 | AT91C_PB14_ERX3 | \
	 AT91C_PB15_ERXDV_ECRSDV | AT91C_PB16_ECOL | AT91C_PB17_ERXCK)

#define RSR_BITS (AT91C_EMAC_BNA | AT91C_EMAC_REC | AT91C_EMAC_OVR)

#define TSR_BITS                                                        \
	(AT91C_EMAC_UBR | AT91C_EMAC_COL | AT91C_EMAC_RLES | AT91C_EMAC_BEX | \
...
static EMACDescriptor rd[EMAC_RECEIVE_DESCRIPTORS] __attribute__((aligned(8)));
static EMACDescriptor td[EMAC_TRANSMIT_DESCRIPTORS] __attribute__((aligned(8)));
...
static void set_address(const uint8_t* p) {
	AT91C_BASE_EMAC->EMAC_SA1L =
			(AT91_REG)((p[3] << 24) | (p[2] << 16) | (p[1] << 8) | p[0]);
...
void mac_lld_start(MACDriver* macp) {
	...
	AT91C_BASE_EMAC->EMAC_RSR =
			AT91C_EMAC_OVR | AT91C_EMAC_REC | AT91C_EMAC_BNA; /* Clears RSR.*/
...
size_t mac_lld_write_transmit_descriptor(MACTransmitDescriptor* tdp,
	...
	if (size > 0) {
		memcpy((uint8_t*)(tdp->physdesc->w1 & W1_T_ADDRESS_MASK) + tdp->offset, buf,
					 size);
		tdp->offset += size;
	}
	return size;
...
void mac_lld_release_transmit_descriptor(MACTransmitDescriptor* tdp) {
	chSysLock();
	tdp->physdesc->w2 =
			(tdp->physdesc->w2 & ~(W2_T_LOCKED | W2_T_USED | W2_T_LENGTH_MASK)) |
			tdp->offset;
...
size_t mac_lld_read_receive_descriptor(MACReceiveDescriptor* rdp,
	...
	if (size > 0) {
		uint8_t* src =
				(uint8_t*)(rdp->physdesc->w1 & W1_R_ADDRESS_MASK) + rdp->offset;
