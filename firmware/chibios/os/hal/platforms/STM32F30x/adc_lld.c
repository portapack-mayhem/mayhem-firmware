...
void adc_lld_init(void) {
	...
	ADCD1.dmamode = ADC_DMA_SIZE | STM32_DMA_CR_PL(STM32_ADC_ADC12_DMA_PRIORITY) |
									STM32_DMA_CR_DIR_P2M | STM32_DMA_CR_MINC | STM32_DMA_CR_TCIE |
	...
	ADCD3.dmamode = ADC_DMA_SIZE | STM32_DMA_CR_PL(STM32_ADC_ADC12_DMA_PRIORITY) |
									STM32_DMA_CR_DIR_P2M | STM32_DMA_CR_MINC | STM32_DMA_CR_TCIE |
									STM32_DMA_CR_DMEIE | STM32_DMA_CR_TEIE;
...
void adc_lld_start(ADCDriver* adcp) {
	...
	if (adcp->state == ADC_STOP) {
		...
		if (&ADCD1 == adcp) {
			bool_t b;
			b = dmaStreamAllocate(adcp->dmastp, STM32_ADC_ADC12_DMA_IRQ_PRIORITY,
		...
		if (&ADCD3 == adcp) {
			bool_t b;
			b = dmaStreamAllocate(adcp->dmastp, STM32_ADC_ADC34_DMA_IRQ_PRIORITY,
														(stm32_dmaisr_t)adc_lld_serve_dma_interrupt,
														(void*)adcp);
...
void adc_lld_start_conversion(ADCDriver* adcp) {
	...
	ccr =
			grpp->ccr | (adcp->adcc->CCR & (ADC_CCR_CKMODE_MASK | ADC_CCR_MDMA_MASK));
	...
	dmaStreamSetTransactionSize(
			adcp->dmastp, ((uint32_t)grpp->num_channels / 2) * (uint32_t)adcp->depth);
#else
	dmaStreamSetTransactionSize(
