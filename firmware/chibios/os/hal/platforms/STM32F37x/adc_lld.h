 ...
 */
#define STM32_ADC_USE_SDADC \
	(STM32_ADC_USE_SDADC1 || STM32_ADC_USE_SDADC2 || STM32_ADC_USE_SDADC3)
...
typedef struct {
	 ...
	 *         union to use is determined by the ADCDriver.  If the ADCDriver adc
	 * parm is not NULL, then use the adc struct, otherwise if the ADCDriver sdadc
	 * parm is not NULL, then use the sdadc struct.
	 */
	union {
...
#define ADC_SMPR1_SMP_SENSOR(n)              \
	((n) << 18) /**< @brief Temperature Sensor \
									 sampling time.             */
#define ADC_SMPR1_SMP_VREF(n)                                           \
	((n) << 21)															/**< @brief Voltage Reference \
																							 sampling time.             */
