...
#define SDADC_FORBIDDEN_CR1_FLAGS                                          \
	(SDADC_CR1_INIT | SDADC_CR1_RDMAEN | SDADC_CR1_RSYNC | SDADC_CR1_JSYNC | \
	 SDADC_CR1_ROVRIE | SDADC_CR1_REOCIE | SDADC_CR1_JEOCIE | SDADC_CR1_EOCALIE)

#define SDADC_ENFORCED_CR1_FLAGS (SDADC_CR1_JDMAEN | SDADC_CR1_JOVRIE)

#define SDADC_FORBIDDEN_CR2_FLAGS                                           \
	(SDADC_CR2_RSWSTART | SDADC_CR2_RCONT | SDADC_CR2_RCH | SDADC_CR2_JCONT | \
	 SDADC_CR2_STARTCALIB | SDADC_CR2_CALIBCNT)
...
void adc_lld_init(void) {
	...
	ADCD1.dmamode = STM32_DMA_CR_CHSEL(ADC1_DMA_CHANNEL) |
									...
									STM32_DMA_CR_DIR_P2M | STM32_DMA_CR_MSIZE_HWORD |
									STM32_DMA_CR_PSIZE_HWORD | STM32_DMA_CR_MINC |
									STM32_DMA_CR_TCIE | STM32_DMA_CR_DMEIE | STM32_DMA_CR_TEIE;
...
void adc_lld_start(ADCDriver* adcp) {
	...
	if (adcp->state == ADC_STOP) {
...
#if STM32_ADC_USE_SDADC1
		if (&SDADCD1 == adcp) {
			bool_t b = dmaStreamAllocate(
					adcp->dmastp, STM32_ADC_SDADC1_DMA_IRQ_PRIORITY,
		...
		if (&SDADCD2 == adcp) {
			bool_t b = dmaStreamAllocate(
					adcp->dmastp, STM32_ADC_SDADC2_DMA_IRQ_PRIORITY,
					(stm32_dmaisr_t)adc_lld_serve_dma_interrupt, (void*)adcp);
		...
		if (&SDADCD3 == adcp) {
			bool_t b = dmaStreamAllocate(
					adcp->dmastp, STM32_ADC_SDADC3_DMA_IRQ_PRIORITY,
					(stm32_dmaisr_t)adc_lld_serve_dma_interrupt, (void*)adcp);
...
void adc_lld_start_conversion(ADCDriver* adcp) {
	...
	dmaStreamSetTransactionSize(
			adcp->dmastp, (uint32_t)grpp->num_channels * (uint32_t)adcp->depth);
...
#if STM32_ADC_USE_ADC
	{
		...
		adcp->adc->SQR1 = grpp->u.adc.sqr[0] | ADC_SQR1_NUM_CH(grpp->num_channels);
	...
	{
		uint32_t cr2 =
				(grpp->u.sdadc.cr2 & ~SDADC_FORBIDDEN_CR2_FLAGS) | SDADC_CR2_ADON;
...
void adcSTM32Calibrate(ADCDriver* adcp) {
	chDbgAssert((adcp->state == ADC_READY) || (adcp->state == ADC_COMPLETE) ||
...
#if STM32_ADC_USE_SDADC
	{
		...
		adcp->sdadc->CR2 =
				(adcp->sdadc->CR2 & ~SDADC_CR2_CALIBCNT) | SDADC_CR2_CALIBCNT_1;
