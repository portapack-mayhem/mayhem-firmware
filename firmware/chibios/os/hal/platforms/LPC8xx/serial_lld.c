...
static const SerialConfig default_config = {
		SERIAL_DEFAULT_BITRATE, (CFG_DL8 | CFG_NOPARITY | CFG_STOP1)};
...
static void serve_interrupt(SerialDriver* sdp) {
	...
	while (u->INTSTAT) {
		...
		if (u->INTSTAT &
				(STAT_OVERRUN | STAT_DELTARXBRK | STAT_FRAMERR | STAT_PARITYERR)) {
			IOREG32 stat = u->STAT;
			set_error(sdp, stat);
			u->STAT = stat;
		}
...
void sd_lld_start(SerialDriver* sdp, const SerialConfig* config) {
	...
	sdp->uart->INTENSET = STAT_FRAMERR | STAT_OVERRUN | STAT_PARITYERR |
												STAT_DELTARXBRK | STAT_RXRDY;
