...
static size_t write(void* ip, const uint8_t* bp, size_t n) {
	return chOQWriteTimeout(&((SerialUSBDriver*)ip)->oqueue, bp, n,
													TIME_INFINITE);
}

static size_t read(void* ip, uint8_t* bp, size_t n) {
	return chIQReadTimeout(&((SerialUSBDriver*)ip)->iqueue, bp, n, TIME_INFINITE);
}
...
static const struct SerialUSBDriverVMT vmt = {write, read, put,		 get,
...
static void inotify(GenericQueue* qp) {
	...
	if (!usbGetReceiveStatusI(sdup->config->usbp, sdup->config->bulk_out) &&
		...
		n = (n / maxsize) * maxsize;
		usbPrepareQueuedReceive(sdup->config->usbp, sdup->config->bulk_out,
														&sdup->iqueue, n);
...
static void onotify(GenericQueue* qp) {
	...
	if (!usbGetTransmitStatusI(sdup->config->usbp, sdup->config->bulk_in) &&
		...
		usbPrepareQueuedTransmit(sdup->config->usbp, sdup->config->bulk_in,
														 &sdup->oqueue, n);
