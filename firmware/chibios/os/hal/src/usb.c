	...
	switch (((usbp->setup[0] & (USB_RTYPE_RECIPIENT_MASK | USB_RTYPE_TYPE_MASK)) |
		...
		case USB_RTYPE_RECIPIENT_ENDPOINT | (USB_REQ_CLEAR_FEATURE << 8):
			...
			if (usbp->setup[2] != USB_FEATURE_ENDPOINT_HALT)
				return FALSE;
			/* Clearing the EP status, not valid for EP0, it is ignored in that
			 * case.*/
...
void usbInitEndpointI(USBDriver* usbp,
	...
	chDbgAssert(usbp->state == USB_ACTIVE, "usbEnableEndpointI(), #1",
							"invalid state");
	chDbgAssert(usbp->epc[ep] == NULL, "usbEnableEndpointI(), #2",
							"already initialized");
...
void usbDisableEndpointsI(USBDriver* usbp) {
	...
	chDbgAssert(usbp->state == USB_SELECTED, "usbDisableEndpointsI(), #1",
							"invalid state");
