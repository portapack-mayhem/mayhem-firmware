...
static bool_t mmc_read(void* instance,
											 uint32_t startblk,
											 uint8_t* buffer,
											 uint32_t n);
static bool_t mmc_write(void* instance,
												uint32_t startblk,
...
bool_t mmcConnect(MMCDriver* mmcp) {
	...
	/* Try to detect if this is a high capacity card and switch to block
		 ...
		 by ElmChan.*/
	if (send_command_R3(mmcp, MMCSD_CMD_SEND_IF_COND, MMCSD_CMD8_PATTERN, r3) !=
			0x05) {
		/* Switch to SDHC mode.*/
		i = 0;
		while (TRUE) {
			if ((send_command_R1(mmcp, MMCSD_CMD_APP_CMD, 0) == 0x01) &&
					(send_command_R3(mmcp, MMCSD_CMD_APP_OP_COND, 0x400001aa, r3) ==
					 0x00))
				break;
	...
	/* Setting block size.*/
	if (send_command_R1(mmcp, MMCSD_CMD_SET_BLOCKLEN, MMCSD_BLOCK_SIZE) != 0x00)
		goto failed;
...
bool_t mmcStartSequentialRead(MMCDriver* mmcp, uint32_t startblk) {
	...
	chDbgAssert(mmcp->state == BLK_READY, "mmcStartSequentialRead(), #1",
							"invalid state");
...
bool_t mmcStopSequentialRead(MMCDriver* mmcp) {
	static const uint8_t stopcmd[] = {
			0x40 | MMCSD_CMD_STOP_TRANSMISSION, 0, 0, 0, 0, 1, 0xFF};
...
bool_t mmcStartSequentialWrite(MMCDriver* mmcp, uint32_t startblk) {
	...
	chDbgAssert(mmcp->state == BLK_READY, "mmcStartSequentialWrite(), #1",
							"invalid state");
	...
	if (mmcp->block_addresses)
		...
		send_hdr(mmcp, MMCSD_CMD_WRITE_MULTIPLE_BLOCK, startblk * MMCSD_BLOCK_SIZE);
