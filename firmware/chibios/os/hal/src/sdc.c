...
bool_t sdcConnect(SDCDriver* sdcp) {
	...
	/* Asks for the RCA.*/
	if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_SEND_RELATIVE_ADDR, 0,
																 &sdcp->rca))
		goto failed;

	/* Reads CSD.*/
	if (sdc_lld_send_cmd_long_crc(sdcp, MMCSD_CMD_SEND_CSD, sdcp->rca, sdcp->csd))
		goto failed;
	...
	if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_SEL_DESEL_CARD, sdcp->rca,
																 resp))
		goto failed;

	/* Block length fixed at 512 bytes.*/
	if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_SET_BLOCKLEN, MMCSD_BLOCK_SIZE,
																 resp) ||
			MMCSD_R1_ERROR(resp[0]))
		goto failed;
	...
	switch (sdcp->cardmode & SDC_MODE_CARDTYPE_MASK) {
		...
		case SDC_MODE_CARDTYPE_SDV20:
			...
			if (sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_APP_CMD, sdcp->rca,
																		 resp) ||
					MMCSD_R1_ERROR(resp[0]))
				goto failed;
 ...
 * @api
 */
bool_t sdcWrite(SDCDriver* sdcp,
								uint32_t startblk,
								const uint8_t* buf,
								uint32_t n) {
	bool_t status;
...
sdcflags_t sdcGetAndClearErrors(SDCDriver* sdcp) {
	...
	chDbgAssert(sdcp->state == BLK_READY, "sdcGetAndClearErrors(), #1",
							"invalid state");
...
bool_t sdcErase(SDCDriver* sdcp, uint32_t startblk, uint32_t endblk) {
	...
	if ((sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_ERASE_RW_BLK_START, startblk,
																	resp) != CH_SUCCESS) ||
			MMCSD_R1_ERROR(resp[0]))
		goto failed;

	if ((sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_ERASE_RW_BLK_END, endblk,
																	resp) != CH_SUCCESS) ||
			MMCSD_R1_ERROR(resp[0]))
		goto failed;

	if ((sdc_lld_send_cmd_short_crc(sdcp, MMCSD_CMD_ERASE, 0, resp) !=
			 CH_SUCCESS) ||
			MMCSD_R1_ERROR(resp[0]))
		goto failed;
