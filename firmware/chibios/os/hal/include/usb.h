 ...
 */
#define USB_DESC_BCD(bcd) (uint8_t)((bcd)&255), (uint8_t)(((bcd) >> 8) & 255)

/**
 * @brief   Device Descriptor helper macro.
 */
#define USB_DESC_DEVICE(bcdUSB, bDeviceClass, bDeviceSubClass,                \
												bDeviceProtocol, bMaxPacketSize, idVendor, idProduct, \
												bcdDevice, iManufacturer, iProduct, iSerialNumber,    \
												bNumConfigurations)                                   \
	USB_DESC_BYTE(18), USB_DESC_BYTE(USB_DESCRIPTOR_DEVICE),                    \
			USB_DESC_BCD(bcdUSB), USB_DESC_BYTE(bDeviceClass),                      \
			USB_DESC_BYTE(bDeviceSubClass), USB_DESC_BYTE(bDeviceProtocol),         \
			USB_DESC_BYTE(bMaxPacketSize), USB_DESC_WORD(idVendor),                 \
			USB_DESC_WORD(idProduct), USB_DESC_BCD(bcdDevice),                      \
			USB_DESC_INDEX(iManufacturer), USB_DESC_INDEX(iProduct),                \
...
#define USB_DESC_CONFIGURATION(wTotalLength, bNumInterfaces,              \
	...
	USB_DESC_BYTE(9), USB_DESC_BYTE(USB_DESCRIPTOR_CONFIGURATION),          \
			USB_DESC_WORD(wTotalLength), USB_DESC_BYTE(bNumInterfaces),         \
			USB_DESC_BYTE(bConfigurationValue), USB_DESC_INDEX(iConfiguration), \
			USB_DESC_BYTE(bmAttributes), USB_DESC_BYTE(bMaxPower)
...
#define USB_DESC_INTERFACE(bInterfaceNumber, bAlternateSetting, bNumEndpoints, \
	...
	USB_DESC_BYTE(9), USB_DESC_BYTE(USB_DESCRIPTOR_INTERFACE),                   \
			USB_DESC_BYTE(bInterfaceNumber), USB_DESC_BYTE(bAlternateSetting),       \
			USB_DESC_BYTE(bNumEndpoints), USB_DESC_BYTE(bInterfaceClass),            \
			USB_DESC_BYTE(bInterfaceSubClass), USB_DESC_BYTE(bInterfaceProtocol),    \
...
#define USB_DESC_INTERFACE_ASSOCIATION(bFirstInterface, bInterfaceCount,  \
																			 bFunctionClass, bFunctionSubClass, \
																			 bFunctionProcotol, iInterface)     \
	USB_DESC_BYTE(8), USB_DESC_BYTE(USB_DESCRIPTOR_INTERFACE_ASSOCIATION),  \
			USB_DESC_BYTE(bFirstInterface), USB_DESC_BYTE(bInterfaceCount),     \
			USB_DESC_BYTE(bFunctionClass), USB_DESC_BYTE(bFunctionSubClass),    \
...
#define USB_DESC_ENDPOINT(bEndpointAddress, bmAttributes, wMaxPacketSize, \
													bInterval)                                      \
	USB_DESC_BYTE(7), USB_DESC_BYTE(USB_DESCRIPTOR_ENDPOINT),               \
			USB_DESC_BYTE(bEndpointAddress), USB_DESC_BYTE(bmAttributes),       \
			USB_DESC_WORD(wMaxPacketSize), USB_DESC_BYTE(bInterval)
/** @} */
