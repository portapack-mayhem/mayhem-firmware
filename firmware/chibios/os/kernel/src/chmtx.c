		...
		while (tp->p_prio < ctp->p_prio) {
			...
			switch (tp->p_state) {
				case THD_STATE_WTMTX:
					...
					continue;
#if CH_USE_CONDVARS || (CH_USE_SEMAPHORES && CH_USE_SEMAPHORES_PRIORITY) || \
...
Mutex* chMtxUnlock(void) {
	...
	chSysLock();
	chDbgAssert(ctp->p_mtxlist != NULL, "chMtxUnlock(), #1",
							"owned mutexes list empty");
	chDbgAssert(ctp->p_mtxlist->m_owner == ctp, "chMtxUnlock(), #2",
							"ownership failure");
...
Mutex* chMtxUnlockS(void) {
	...
	chDbgCheckClassS();
	chDbgAssert(ctp->p_mtxlist != NULL, "chMtxUnlockS(), #1",
							"owned mutexes list empty");
	chDbgAssert(ctp->p_mtxlist->m_owner == ctp, "chMtxUnlockS(), #2",
							"ownership failure");
