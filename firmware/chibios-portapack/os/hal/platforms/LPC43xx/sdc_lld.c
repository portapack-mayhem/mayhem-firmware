...
static const sdio_resources_t sdio_resources = {
		.base = {.clk = &LPC_CGU->BASE_SDIO_CLK,
						 .stat = &LPC_CCU2->BASE_STAT,
						 .stat_mask = 0},
		.branch_register_if = {.cfg = &LPC_CCU1->CLK_M4_SDIO_CFG,
													 .stat = &LPC_CCU1->CLK_M4_SDIO_STAT},
		.branch_peripheral = {.cfg = &LPC_CCU2->CLK_SDIO_CFG,
													.stat = &LPC_CCU2->CLK_SDIO_STAT},
		.reset = {.output_index = 20},
		.interrupt = {.irq = SDIO_IRQn,
									.priority_mask =
...
static void sdio_status_response_error_clear(void) {
	LPC_SDMMC->RINTSTS |= (1U << 1)		/* RE: Response error */
												| (1U << 6) /* RCRC: Response CRC error */
												| (1U << 8) /* RTO: Response time-out error */
			;
}
...
static void sdio_update_clock_registers_only(void) {
	LPC_SDMMC->CMDARG = 0;
	LPC_SDMMC->CMD = (1U << 13)		/* WAIT_PRVDATA_COMPLETE */
...
static void sdio_clkdiv_set(const size_t divider_0) {
...
static void sdio_width_set_1bit(void) {
	LPC_SDMMC->CTYPE = (0U << 0)		/* CARD_WIDTH0 */
										 | (0U << 16) /* CARD_WIDTH1 */
			;
}

static void sdio_width_set_4bit(void) {
	LPC_SDMMC->CTYPE = (1U << 0)		/* CARD_WIDTH0 */
										 | (0U << 16) /* CARD_WIDTH1 */
			;
}
...
static void sdio_dma_interrupts_clear(void) {
	LPC_SDMMC->IDSTS = (1U << 0)	 /* TI: Transmit interrupt */
...
static bool_t sdc_llc_prepare_descriptors_chained(LPC_SDMMC_DESC_Type desc[],
	...
	for (uint32_t i = 0; i < desc_count; i++) {
		uint32_t buffer_1_size =
				(byte_count > buffer_size) ? buffer_size : byte_count;
		desc[i].DESC2 = p;	//(buffer_1_size == 0) ? 0 : p;
		p += buffer_1_size;
		byte_count -= buffer_1_size;

		uint32_t buffer_2_size = 0;
		desc[i].DESC1 = (buffer_1_size << 0) | (buffer_2_size << 13);
 ...
 */
static bool_t sdc_lld_wait_transaction_end(SDCDriver* sdcp,
																					 uint32_t n,
																					 uint32_t* resp) {
	...
	if (LPC_SDMMC->INTMASK != 0) {
		chDbgAssert(sdcp->thread == NULL, "sdc_lld_start_data_transaction(), #1",
								"not NULL");
		sdcp->thread = chThdSelf();
		chSchGoSleepS(THD_STATE_SUSPENDED);
		chDbgAssert(sdcp->thread == NULL, "sdc_lld_start_data_transaction(), #2",
								"not NULL");
	}
...
static bool_t sdio_send_command(SDCDriver* sdcp,
																const uint32_t cmd_and_flags,
																const uint32_t arg) {
	LPC_SDMMC->CMDARG = arg;
	LPC_SDMMC->CMD = cmd_and_flags | (1U << 13) /* WAIT_PRVDATA_COMPLETE */
									 | (1U << 31)								/* START_CMD */
			;
...
static bool_t sdc_lld_send_cmd_data(SDCDriver* sdcp,
																		uint32_t cmd_and_flags,
																		uint32_t arg,
																		uint32_t* resp) {
	(void)sdcp;
	...
	const uint32_t status = sdio_send_command(
			...
			cmd_and_flags | (1U << 6) | (1U << 8) | (1U << 9), arg);
...
static bool_t sdc_lld_send_cmd_data_read(SDCDriver* sdcp,
																				 uint8_t cmd,
																				 uint32_t arg,
																				 uint32_t* resp) {
...
static bool_t sdc_lld_send_cmd_data_write(SDCDriver* sdcp,
																					uint8_t cmd,
																					uint32_t arg,
																					uint32_t* resp) {
 ...
 * @notapi
 */
static bool_t sdc_lld_prepare_read(SDCDriver* sdcp,
																	 uint32_t startblk,
																	 uint32_t n,
																	 uint32_t* resp) {
	...
	if (n > 1) {
		...
		if (sdc_lld_send_cmd_data_read(sdcp, MMCSD_CMD_READ_SINGLE_BLOCK, startblk,
																	 resp) ||
				MMCSD_R1_ERROR(resp[0]))
			return CH_FAILED;
	}
 ...
 * @notapi
 */
static bool_t sdc_lld_prepare_write(SDCDriver* sdcp,
																		uint32_t startblk,
																		uint32_t n,
																		uint32_t* resp) {
	/* Driver handles data in 512 bytes blocks (just like HC cards). But if we
	...
	if (n > 1) {
		...
		if (sdc_lld_send_cmd_data_write(sdcp, MMCSD_CMD_WRITE_BLOCK, startblk,
																		resp) ||
				MMCSD_R1_ERROR(resp[0]))
			return CH_FAILED;
	}
...
void sdc_lld_start(SDCDriver* sdcp) {
	if (sdcp->state == BLK_STOP) {
		...
		//       2        OK       OK       OK             1
		//       3        OK       OK       OK             1
		//       4        OK     f_read 1   0x2      3     2 (20170424 fails,
		//       20170522 OK) 5        OK     f_read 1   0x2      1
		...
		LPC_SCU->SDDELAY =
				...
				;
		LPC_SDMMC->CTRL = (1U << 4)		 /* INT_ENABLE */
		...
		LPC_SDMMC->BMOD = (4U << 2)		/* Descriptor skip length = 4 */
											| (1U << 7) /* DE: DMA Enable */
											| (1U << 8) /* Burst length = 4 */
				;
...
bool_t sdc_lld_send_cmd_short(SDCDriver* sdcp,
	...
	const uint32_t status = sdio_send_command(sdcp,
																						/* RESPONSE_EXPECT */
																						cmd | (1U << 6), arg);
 ...
 * @notapi
 */
bool_t sdc_lld_read_aligned(SDCDriver* sdcp,
														uint32_t startblk,
														uint8_t* buf,
														uint32_t n) {
	chDbgCheck((n <= (LPC_SDC_SDIO_DESCRIPTOR_COUNT *
										LPC_SDC_SDIO_MAX_DESCRIPTOR_BYTES / MMCSD_BLOCK_SIZE)),
	...
	if (sdc_llc_prepare_descriptors_chained(desc, sizeof(desc) / sizeof(desc[0]),
																					buf, n * MMCSD_BLOCK_SIZE) == TRUE)
		goto error;
 ...
 * @notapi
 */
bool_t sdc_lld_write_aligned(SDCDriver* sdcp,
														 uint32_t startblk,
														 const uint8_t* buf,
														 uint32_t n) {
	chDbgCheck((n <= (LPC_SDC_SDIO_DESCRIPTOR_COUNT *
