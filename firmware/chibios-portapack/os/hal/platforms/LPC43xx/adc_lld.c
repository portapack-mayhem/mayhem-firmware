...
static const adc_resources_t adc0_resources = {
		.base = {.clk = &LPC_CGU->BASE_APB3_CLK,
						 ...
						 .stat_mask = (1U << 0)},
		.branch = {.cfg = &LPC_CCU1->CLK_APB3_ADC0_CFG,
							 .stat = &LPC_CCU1->CLK_APB3_ADC0_STAT},
		.reset = {.output_index = 40},
		.interrupt = {.irq = ADC0_IRQn,
									.priority_mask =
											CORTEX_PRIORITY_MASK(LPC43XX_ADC0_IRQ_PRIORITY)},
};
...
static const adc_resources_t adc1_resources = {
		.base = {.clk = &LPC_CGU->BASE_APB3_CLK,
						 .stat = &LPC_CCU1->BASE_STAT,
						 .stat_mask = (1U << 0)},
		.branch = {.cfg = &LPC_CCU1->CLK_APB3_ADC1_CFG,
							 .stat = &LPC_CCU1->CLK_APB3_ADC1_STAT},
		.reset = {.output_index = 41},
		.interrupt = {.irq = ADC1_IRQn,
									.priority_mask =
											CORTEX_PRIORITY_MASK(LPC43XX_ADC1_IRQ_PRIORITY)},
};
...
void adc_lld_start(ADCDriver* adcp) {
	if (adcp->state == ADC_STOP) {
		...
		adcp->adc->CR = (0x00 << 0) | (adcp->config->cr_clkdiv << 8) | (0 << 16) |
										(adcp->config->cr_clks << 17) | (1 << 21) | (0 << 24) |
										(0 << 27);
	}
}
...
void adc_lld_stop(ADCDriver* adcp) {
	if (adcp->state == ADC_READY) {
		...
		adcp->adc->CR = (0x00 << 0) | (adcp->config->cr_clkdiv << 8) | (0 << 16) |
										(adcp->config->cr_clks << 17) | (0 << 21) | (0 << 24) |
										(0 << 27);
...
void adc_lld_start_conversion(ADCDriver* adcp) {
	...
	adcp->adc->CR = (grpp->cr_sel << 0) | (adcp->config->cr_clkdiv << 8) |
									(0 << 16) | (adcp->config->cr_clks << 17) | (1 << 21) |
									(1 << 24) | (0 << 27);
}
