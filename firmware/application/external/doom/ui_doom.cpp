...
void initialize_level() {
    ...
    while (initial_enemies < 2 && num_entities < MAX_ENTITIES && attempts < 50) {
        ...
        uint8_t spawn_x = (temp_x < 0) ? 0 : (temp_x >= LEVEL_WIDTH) ? LEVEL_WIDTH - 1 : temp_x;
        uint8_t spawn_y = (temp_y < 0) ? 0 : (temp_y >= LEVEL_HEIGHT) ? LEVEL_HEIGHT - 1 : temp_y;
