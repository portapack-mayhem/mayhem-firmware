...
// Provide PROGMEM compatibility for embedded system
...
// Moved to PROGMEM to free up ~936 bytes of RAM - Priority 1.1 Memory Optimization
...
static const uint8_t level[LEVEL_SIZE] PROGMEM = {
...
uint8_t get_block_at(uint8_t x, uint8_t y) {
    ...
    // Access level data from PROGMEM (Priority 1.1 memory optimization)
    const uint8_t nibble = pgm_read_byte(&level[((LEVEL_HEIGHT - 1 - y) * LEVEL_WIDTH + x) / 2]);
    return (nibble >> (!(x % 2) * 4)) & 0b1111;
}
